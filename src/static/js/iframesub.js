...
function onPlayerReady(event) {
	...
	get_subtitle(window);
	console.log("elsubtitles:", elsubtitles);
}
...
function onPlayerStateChange(event) {
	...
	if (event.data == 1) {
		...
		//Need to pause Dub audio when Video pauses
		//Need to seek within audio if video is seeked

		var uservol = player.getVolume();
		...
		myTimer = setInterval(function() {
			...
			if (time.toFixed(1) > elsubtitles[elsubcount][0] && time.toFixed(1) < elsubtitles[elsubcount][1]) {
				//subtitle = elsubtitles[elsubcount][2];
				var subEl = document.getElementById(elsubcount);
				subEl.style.backgroundColor = "salmon";
				if (!done) {
					var dubs = document.getElementById("dub" + elsubcount);
					var subdur = elsubtitles[elsubcount][1] - elsubtitles[elsubcount][0];
					var ratio = Math.round((dubs.duration / subdur) * 100) / 100;
					//FOr stretching or speeding audio to fit
					if (ratio >= 0.5) {
						dubs.playbackRate = ratio;
						console.log("Ratio is ", ratio);
					}//audio is muted for <0.5 thats why
					player.setVolume(0.15 * uservol);
					dubs.play();
					done = true;
				}
			}
			// is there a next timecode?
			if (time.toFixed(1) >= elsubtitles[elsubcount][1] && elsubcount < (elsubtitles.length - 1)) {
				elsubcount++;
				player.setVolume(uservol);
				done = false;
			}
			if (time.toFixed(1) <= elsubtitles[elsubcount][0] && elsubcount < (elsubtitles.length - 1)) {
				elsubcount--;
				player.setVolume(uservol);
				done = false;
			}
		}, 100);
		// 100 means repeat in 100 ms
	} else {// not playing
		clearInterval(myTimer);
	}
}//closing bracket for OnplayerStateChange

// check for video tags and show subtitle track if the browser doesn't know how
function get_subtitle(w) {
	...
	(function(a) {
		...
		function g(a) {
			var c = b["$" + a] || window[a];
			if (!c)
				throw new Error("Ender Error: Requested module '" + a + "' has not been defined.");
			return c
		}
		a.global = a;
		var b = {},
		    ...
		    e = a.require,
		    f = a.provide;
		a.provide = h, a.require =
		g, j.prototype.forEach = function(a, b) {
			var c,
			    d;
			for ( c = 0,
			d = this.length; c < d; ++c)
				c in this && a.call(b || this[c], this[c], c, this);
			return this
		}, j.prototype.$ =
		k, k._VERSION = "0.4.3-dev", k.fn = j.prototype, k.ender = function(a, b) {
			i( b ? j.prototype : k, a)
			...
			a.$ = c, b && (a.provide = f, a.require =
			e, a.ender =
			d, typeof b == "function" && b(g, h, this));
			return this
		...
		! function(a, b) {
			...
			function eachFormElement() {
				...
				for ( c = 0; c < arguments.length; c++)
					b = arguments[c], /input|select|textarea/i.test(b.tagName) && serial(b, a), e(b, ["input", "select", "textarea"])
			}
			...
			function getRequest(a, b, c) {
				var d = (a.method || "GET").toUpperCase(),
				    ...
				    g;
				(a.type == "jsonp" || d == "GET") && f && ( e = urlappend(e, f),
				f = null);
				if (a.type == "jsonp")
					return handleJsonp(a, b, c, e);
			...
			function handleJsonp(a, b, c, d) {
				var e = uniqid++,
				    ...
				    k = 0;
				i ? i[3] === "?" ? d = d.replace(h, "$1=" + g) : g = i[3] : d = urlappend(d, f + "=" + g), win[g] =
				generalCallback, j.type = "text/javascript", j.src =
				d, j.async = !0, typeof j.onreadystatechange != "undefined" && (j.event = "onclick", j.htmlFor = j.id = "_reqwest_" + e), j.onload = j.onreadystatechange = function() {
					if (j[readyState] && j[readyState] !== "complete" && j[readyState] !== "loaded" || k)
						return !1;
					j.onload = j.onreadystatechange = null, j.onclick && j.onclick(), a.success && a.success(lastValue),
					lastValue =
					undefined, head.removeChild(j),
					k = 1
				}, head.appendChild(j)
			}
		...
		! function(b, c, d) {
			...
			var c = window,
				...
				return function(r, s) {
					...
					if (s) {
						if (z.indexOf("key") !== -1)
							y = d, x.keyCode = r.keyCode || r.which;
						else if (t.test(z)) {
							...
							if (r.pageX || r.pageY)
								x.clientX = r.pageX, x.clientY = r.pageY;
							else if (r.clientX || r.clientY)
								x.clientX = r.clientX + o.body.scrollLeft + p.scrollLeft, x.clientY = r.clientY + o.body.scrollTop + p.scrollTop;
							e.test(z) && (x.relatedTarget = r.relatedTarget || r[(z === "mouseover" ? "from" : "to") + "Element"])
						} else
							w.test(z) ? y = g : u.test(z) ? y = c : v.test(z) ? y = f : z === "message" && ( y = h);
						q(r, x, y || a)
					}
					return x
				}
				...
				function a(a, b, c, d, e) {
					...
					this.element = a, this.type =
					b, this.handler =
					c, this.original =
					d, this.namespaces =
					e, this.custom = z[b], this.eventType = q || f ? b : "propertychange", this.customType = !q && !f && b, this[m] = B(a, f), this[r] = this[m][r]
				}
				a.prototype = {
				...
				var h = b.__beanDel,
					...
					if ( e ? e.apply(j, arguments) : q ? !0 : i && i.propertyName === "_on" + d || !i)
						i && ( i = A(i || ((this[l] || this.document || this).parentWindow || c).event, g), i.currentTarget =
						j), b.apply(a, i && (!f || f.length === 0) ? arguments : s.call(arguments, i ? 0 : 1).concat(f))
				};
				i.__beanDel = h;
				return i
			},
				...
				for ( e = 0,
				f = j.length; e < f; e++)
					j[e].inNamespaces(d) && ((h=j[e])[r] && G(h[m], h.eventType, h.handler, !1, h.type), D.del(h))
			},
				...
				if (D.has(a, i, c))
					return a;
				i === "unload" && ( c = J(K, a, i, c, d)), z[i] && (z[i].condition && ( c = I(a, c, i, z[i].condition, e, !0)),
				i = z[i].base || i),
				h = D.put(new C(a, i, c, d, j[0] && j)), h.handler = h.isNative ? H(a, h.handler, e) : I(a, h.handler, i, !1, e, !1), h[r] && G(h[m], h.eventType, h.handler, !0, h.customType)
			},
			    M = function(a, b, c) {
				var d = function(b, d) {
					var e,
					    f = typeof a == "string" ? c(a, d) : a;
				...
				if (b && !c && typeof b == "object")
					...
					i = arguments.length > 3 ? s.call(arguments, 3) : [],
					g = ( k ? c : b).split(" "), k && ( c = M(b, j = d, e || E)) && ( i = s.call(i, 1)), this === x && ( c = J(N, a, b, c, j));
					for ( h = g.length; h--; )
						L(a, g[h], c, j, i)
				}
				return a
				...
				var d = 0,
				    ...
				    h;
				for (; d < f; d++)
					e[d].original && ( h = e[d].handler.__beanDel, h ? g = [a, h.selector, e[d].type, e[d].original, h.$] : g = [a, e[d].type, e[d].original], O.apply(null, g));
				return a
			},
			    T = {
				add : O,
			...
			var b = require("bean"),
				...
				return function() {
					for (var d = 0,
					    f = this.length; d < f; d++)
						!arguments.length && a == "add" && c && ( a = "fire"), b[a].apply(this, [this[d]].concat(e, Array.prototype.slice.call(arguments, 0)));
					return this
				}
			},
			    d = c("add"),
				...
				hover : function(a, c, d) {
					for ( d = this.length; d--; )
						b.add.call(this, this[d], "mouseenter", a), b.add.call(this, this[d], "mouseleave", c);
					return this
				}
			},
		...
		(function(b, c, d) {
			...
			function X(b, c, d) {
				...
				if (!e)
					return this;
				if (b == null && c == null)
					return (Y(e)?Z():{x:e.scrollLeft,y:e.scrollTop})[d];
			...
			function S(a, b, c) {
				...
				e == "static" && (d.css("position", g),
				e =
				g), isNaN(i[0]) && (i[0] = h ? 0 : a.offsetLeft), isNaN(i[1]) && (i[1] = h ? 0 : a.offsetTop), b != null && (a.style.left = b - f.left + i[0] + v), c != null && (a.style.top = c - f.top + i[1] + v)
			}
			...
			function H(a, b, c) {
				for (var d = 0,
				    e = a.length; d < e; d++)
					N(a[d]) && (H(a[d].childNodes, b, c), b.call(c || a[d], a[d], d, a));
				return a
			}
			...
			function G(a, b, c, d) {
				var e,
				    ...
				    g = a.length;
				for (; f < g; f++)
					e = d ? a.length - f - 1 : f, b.call(c || a[e], a[e], e, a);
				return a
			}
			...
			q = function(a, b) {
				...
				map : function(a, b) {
					...
					for ( e = 0; e < this.length; e++)
						d = a.call(this, this[e], e), b ? b(d) && c.push(d) : c.push(d);
					return c
				},
				html : function(a, b) {
					var d = b ? c.textContent === undefined ? "innerText" : "textContent" : "innerHTML",
					    ...
					    h = function(c, e) {
						try {
							if (b || typeof a == "string" && !g.test(c.tagName))
								return c[d] = a
						} catch(h) {
						}
						f(c, e)
				...
				css : function(c, d) {
					...
					d), n && f.opacity && (f.filter = "alpha(opacity=" + f.opacity * 100 + ")", f.zoom = c.zoom || 1,
					delete f.opacity);
					return this.each(g)
				},
				offset : function(a, c) {
					...
					var d = this[0],
					    ...
					    g = d.offsetTop,
					    h = d.offsetLeft;
					while ( d = d.offsetParent) g = g + d.offsetTop,
					h = h + d.offsetLeft, d != b.body && (g -= d.scrollTop, h -= d.scrollLeft);
					return {
						top : g,
						left : h,
						height : f,
				...
				return typeof a == "string" && a !== "" ? function() {
					...
					do (!c || e.nodeType == 1) && (!k || e.tagName.toLowerCase() != "tbody") && f.push(e);
			...
			return $
		}, this), provide("bonzo", a.exports), function(a) {
			function e(a, c) {
		...
		(function(b, c, d) {
			...
			function Z(a, b) {
				return function(c) {
					var d,
					    e;
					p.test(c) ? a[h] !== 9 && (( e = d = a.getAttribute("id")) || a.setAttribute("id", e = "__qwerymeupscotty"),
					c = '[id="' + e + '"]' + c, b(a.parentNode || a, c, !0), d || a.removeAttribute("id")) : c.length && b(a, c, !1)
				}
			}
			...
			function R(a, b, c) {
				if (T(b))
					return a == b;
				if (V(b))
					return !!~J(b).indexOf(a);
				var d = b.split(","),
				    e,
				    f;
				while ( b = d.pop()) {
					e = G.g(b) || G.s(b, b.split(z)),
					f = b.match(y),
					e = e.slice(0);
					if (N.apply(a, M(e.pop())) && (!e.length || S(a, e, f, c)))
						return !0
				}
				return !1
			}
			...
			function Q(a, b) {
				...
				if (!q)
					return c;
				o = M(k),
				n = q !== b && q[h] !== 9 && s && /^[+~]$/.test(s[s.length - 1]) ? function(a) {
			...
			function N(a, b, c, d, e, f, i, l, m, n, o) {
				...
				if (this[h] !== 1)
					return !1;
				if (b && b !== "*" && this[g] && this[g].toLowerCase() !== b)
					return !1;
				if (c && ( q = c.match(j)) && q[1] !== this.id)
					return !1;
				if (c && ( t = c.match(k)))
					for ( p = t.length; p--; )
						if (!H(t[p].slice(1)).test(this.className))
							return !1;
				if (m && Y.pseudos[m] && !Y.pseudos[m](this, o))
					return !1;
				if (d && !i) {
					s = this.attributes;
					for (r in s)
					if (Object.prototype.hasOwnProperty.call(s, r) && (s[r].name || r) == e)
						return this
				}
				if (d && !P(f, _(this, e) || "", i))
					return !1;
			...
			function J(a) {
				for (var b = [],
				    c = 0,
				    d = a.length; c < d; ++c)
					V(a[c]) ? b = b.concat(a[c]) : b[b.length] = a[c];
				return b
			}
			...
			var D = new C,
				...
				if ( f = a.match(o)) {
					...
					for ( g = 0,
					i = e.length; g < i; g++)
						j.test(e[g].className) && (c[c.length] = e[g]);
					return c
				}
			...
			R, Y.pseudos = {};
			return Y
		}, this), provide("qwery", a.exports), function(a) {
			var b = function() {
				var a;
				try {
					a = require("qwery")
			...
			a.pseudos = b.pseudos, a._select = function(c, d) {
				...
				and : function(b) {
					...
					for (var d = this.length,
					    e = 0,
					    f = this.length + c.length; d < f; d++, e++)
						this[d] = c[e];
	...
	function videosub_main() {
		...
		// check for track element method, if it doesn't exist, the browser generally doesn't support track elements
		// first find all video tags
		$VIDEOSUB('iframe').each(function(el) {
			...
			if (el.hasChildNodes()) {
				...
				for (var i = 0; i < children.length; i++) {
					if (children[i].nodeValue.includes('track')) {
						var test = $VIDEOSUB(children[i].nodeValue.match("src=\"(.*)srt"));
						subtitlesrc = test[1] + "srt";
					}
				};
			};
			if (subtitlesrc.indexOf('.srt') != -1) {// we have a track tag and it's a .srt file
				...
				el.update = function(req) {
					...
					if (records.length == 1) {
						records = req.split('\r\n\r\n');
					}//some issue with Enter carriage with srt or windows vs linux files
					for (var r = 0; r < records.length; r++) {
						record = records[r];
						elsubtitles[r] = record.split('\n');
						//each line is stored in array form
					}
					//Optional : Modifying subtitles array by grouping lines together - Chunking
					var a = 0;
					var subDiv = document.createElement("div");

					$VIDEOSUB(subDiv).css({//CSS styling for Subtitles
						'position' : 'absolute',
					...
					for ( i = 0; i < (elsubtitles.length - 2); i++) {
						var oneChunk = "";
						for ( j = i; j < (elsubtitles.length - 2) && videosub_timediff(elsubtitles[i][1], elsubtitles[j][1]) < chunksize; j++) {
							oneChunk += (" " + elsubtitles[j][2]);
							for ( k = 3; elsubtitles[j][k]; k++) {
								oneChunk += (" " + elsubtitles[j][k]);
							} //solves multiline srt file issue
						}
						if (j == i) {
							oneChunk += (" " + elsubtitles[j][2]);
							j++;
						}
						// Need to write a funtion that ensures that Chunk doesn't have only one or two words //
						// Need to color text as it goes from start to end of dialogue, use span color tag n keep extending it //
						// Auto stop recording when line completes//
						//When gaps between dialogues exist in srt's, split them if gap is more than 3 secs, don't combine in a chunk.//
						//On Hover buttons needed - Listen, dub, preview, re-dub, done, delete.. ???? //
						//Include auto-translated subtitles for reference & make it editable //
						//Then use aeneas to detect if some of the words of translated text were spoken or not//

						//To solve overlapped subtitling.
						if (videosub_timecode_max(elsubtitles[j-1][1]) > videosub_timecode_min(elsubtitles[j][1])) {
							elsubtitles[a] = [videosub_timecode_min(elsubtitles[i][1]), videosub_timecode_min(elsubtitles[j][1]), oneChunk];
						} else {
							elsubtitles[a] = [videosub_timecode_min(elsubtitles[i][1]), videosub_timecode_max(elsubtitles[j-1][1]), oneChunk];
						}
						var newPara = document.createElement("P");
						newPara.id = a;
						newPara.innerHTML = elsubtitles[a][2];
						subDiv.appendChild(newPara);
						paraEl.push(document.getElementById(a));
						a++;
						i = j - 1;
					}// Chunking ends.
					elsubtitles.splice(a, elsubtitles.length);
					//Has to remove old extra lines but doesnt work n its okay.

					//Adding Event Listeners to Each Chunk and Recording on CLick.
					var rec = 0;
					// A variable defined to detect that recording is happening cuz button.style == blue isnt working
						...
						document.getElementById(i).addEventListener('click', function() {
							...
							button.mediaCapturedCallback = function() {
								...
								button.recordingEndedCallback = function(url) {
									...
									audio.controls = true;
									//Hide or Unhide player
									recordingPlayer.parentNode.appendChild(document.createElement('hr'));
									recordingPlayer.parentNode.appendChild(audio);

									//if(audio.paused) audio.play(); // Commented out to prevent autoplay
					...
					for (var i = 0; i < elsubtitles.length; i++) {
						listenerForI(i);
					}
...
// Functions that Parse & manipulate subtitles
...
function videosub_timediff(line1, line2) {
	td = (videosub_timecode_max(line2) - videosub_timecode_min(line1));
	return td;
}
